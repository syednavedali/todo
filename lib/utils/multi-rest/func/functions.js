...
f.check = function check(files, options, callback) {
	
	var isok = false;
	
	if (typeof(files.ss) === 'undefined')
	{
		...
		if (options.extensions && !options.extensions.includes(mime.extension(files.ss.type) || files.ss.type.split("/")[1])) {
	...
	if(files.thumb)
	{
		if (typeof(files.ss) === 'undefined')
		{
			...
			if (options.extensions && !options.extensions.includes(mime.extension(files.thumb.type) || files.thumb.type.split("/")[1])) {
				return callback({code: "ExternalError", messege: "Please upload file with allowed extensions."+JSON.stringify(options.extensions)+" - "+files.thumb.type+"- -"+options.extensions.includes(mime.extension(files.thumb.type) || files.thumb.type.split("/")[1])}, null);
