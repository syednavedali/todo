...
function createPost(req, res, next) {
		...
		console.log("Accept Json"+JSON.stringify(req.body));
...
var upload = new Multi({ 
    uploadDir: "C:/Users/snavedal/git/todo/uploads/", 
    filename: 'random', 
    filefields: ['video'], 
    extensions: ['mp4'],
...
function createServer(options) {
    ...
    server.post('/post', upload, createPost);
    //server.post('/post', createPost);
    server.get('/posts', listPost);
    server.head('/posts', listPost);
    server.get('/categories', listCategories);
    server.head('/categories', listCategories);
    server.get('/types', listTypes);
    server.head('/types', listTypes);
